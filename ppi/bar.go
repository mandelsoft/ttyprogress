...
func NewBarBase[T BarImpl[V], V any](self T, p Container, c specs.BarBaseConfiguration, total V, closer func(), tick ...bool) (*BarBase[T, V], *BarBaseImpl[T, V], error) {
	e := &BarBaseImpl[T, V]{
		total:   total,
		width:   c.GetWidth(),
		config:  c.GetConfig(),
...
func (b *BarBaseImpl[T, V]) Visualize() (ttycolors.String, bool) {
	...
	if b.width > 0 {
		...
		completedWidth := int(float64(b.width) * (b.Self().CompletedPercent() / 100.00))
	...
	return ttycolors.Sequence(buf.String()), b.Self().IsFinished()
}

...
func NewProgressBase[T ProgressImpl](self T, p Container, c specs.ProgressConfiguration, view int, closer func(), tick ...bool) (*ProgressBase[T], *ProgressBaseImpl[T], error) {
	e := &ProgressBaseImpl[T]{tick: general.Optional(tick...)}
	e.format = c.GetColor()
	e.progressFormat = c.GetProgressColor()

	for _, def := range c.GetPrependDecorators() {
		d := def.CreateDecorator(self)
